...
MainWindowImpl::MainWindowImpl(QWidget *parent, Qt::WindowFlags f)
  ...
  connect(deviceHandler, SIGNAL(deviceAdded(string)), this,
          SLOT(newDevice(string)));
  connect(rescanButton, SIGNAL(clicked()), deviceHandler, SLOT(rescan()));
  connect(deviceHandler, SIGNAL(rescanFinished()), this,
          SLOT(showAttachedUSBDevices()));
  ...
  connect(deviceCombo, SIGNAL(currentIndexChanged(int)), deviceHandler,
  ...
  connect(passwdLine1, SIGNAL(textChanged(const QString &)), this,
          SLOT(enableSaveButton()));
  connect(passwdLine2, SIGNAL(textChanged(const QString &)), this,
          SLOT(enableSaveButton()));
  ...
  connect(action_Forum_Online, SIGNAL(triggered(bool)), this,
          SLOT(showForum()));
...
void MainWindowImpl::showHelp() {
  ...
  QFile doc(docfile);
  if (!doc.exists()) {
    docfile = QString(DOCDIR) + QString(HELP_BASE) + QString("en") +
              QString(HELP_EXT);
  }
...
bool MainWindowImpl::getUuid(QString dir) {
  ...
  while (true) {
    ...
    // syslog(LOG_DEBUG,"DIR: %s, MP:
    // %s.",dir.toStdString().data(),mountPoint.toStdString().data());
    if (mountPoint != "/" && dir.startsWith(mountPoint)) {
      QString dev = line.section(' ', 0, 0);
      // syslog(LOG_DEBUG,"DIR: %s, DEV:
      // %s.",dir.toStdString().data(),dev.toStdString().data());
      ...
      while (uuidIt.hasNext()) {
        ...
        if (info.isSymLink()) {
          if (info.symLinkTarget() == dev) {
            ...
            // syslog(LOG_DEBUG,"UUID: %s, D:
            // %s.",info.fileName().toStdString().data(),d.fileName().toStdString().data());
            if (d.exists() && d.open(QIODevice::ReadOnly)) {
              QString s = d.readAll();
              // syslog(LOG_DEBUG,"UUID: %s, S:
              // %s.",info.fileName().toStdString().data(),s.toStdString().data());
              if (s.at(0) == '1') {
                mtab.close();
                return true;
              }
...
void MainWindowImpl::savePasswd() {
  ...
  UserSettings userSettings((char *)passwdPath.toStdString().data(),
  ...
  syslog(LOG_DEBUG, "%s for %s to: %s", msg, username.toStdString().data(),
         userSettings.pathToPassword.toStdString().data());
...
void MainWindowImpl::enableSaveButton() {
  passwdReady = true;
  if (passwdUUID.isEmpty() | passwdPath.isEmpty() |
      passwdLine1->text().isEmpty() | passwdLine2->text().isEmpty() |
...
void MainWindowImpl::setPwdPath(QString selectedPath) {
  ...
  if (getUuid(passwdPath)) {
    syslog(LOG_DEBUG, "Selected Dir for Password Store: %s (UUID: %s)",
           pwdDir.absolutePath().toStdString().data(),
           passwdUUID.toStdString().data());
  }
  enableSaveButton();
}
...
void MainWindowImpl::setPwdPath1() {
  QString dir = QFileDialog::getExistingDirectory(
      this, tr("Open Directory on removable Media"), "/media",
      QFileDialog::ShowDirsOnly | QFileDialog::DontResolveSymlinks);
  setPwdPath(dir);
}

void MainWindowImpl::setPwdPath2() { setPwdPath(pathEdit->text()); }
...
void MainWindowImpl::acquireFinished(int result, struct fp_pic_data *pic) {
  ...
  sDialog->show();
  connect(sDialog, SIGNAL(choice(savedDialogChoice)), this,
          SLOT(savedDialogChosen(savedDialogChoice)));
}
...
int MainWindowImpl::saveToFile() { // save fingerprints to tar-file (needs tar
  ...
  QString fileName = QFileDialog::getSaveFileName(
      this, tr("Save Fingerprints"), home, tr("Tar Archives (*.tar.gz)"));
  string command = "cd ~ ;";
...
void MainWindowImpl::tabChanged() { tabChanged(tabWidget->currentIndex()); }
...
void MainWindowImpl::showAttachedUSBDevices() {
  ...
  if (deviceHandler->getAttachedUsbDevices() != nullptr) {
    for (USBDevice *attachedUSB = deviceHandler->getAttachedUsbDevices();
...
void MainWindowImpl::scanTabToBack() {
  movie->setPaused(true);
  if (currentFingerprint != nullptr) {
    disconnect(currentFingerprint->getDevice(), SIGNAL(noDeviceOpen()), this,
               SLOT(noDeviceOpen()));
    disconnect(currentFingerprint->getDevice(), SIGNAL(neededStages(int)), this,
               SLOT(setScanTabNeededStages(int)));
    disconnect(currentFingerprint, SIGNAL(neededStages(int)), this,
               SLOT(setScanTabNeededStages(int)));
    disconnect(currentFingerprint->getDevice(), SIGNAL(acquireResult(int)),
               currentFingerprint, SLOT(newAcquireResult(int)));
    disconnect(currentFingerprint->getDevice(),
               SIGNAL(verifyResult(int, struct fp_pic_data *)),
               currentFingerprint,
               SLOT(newVerifyResult(int, struct fp_pic_data *)));
    disconnect(currentFingerprint,
               SIGNAL(acquireFinished(int, struct fp_pic_data *)), this,
               SLOT(acquireFinished(int, struct fp_pic_data *)));
    disconnect(currentFingerprint,
               SIGNAL(verifyFinished(int, struct fp_pic_data *)), this,
               SLOT(verifyFinished(int, struct fp_pic_data *)));
    delete (currentFingerprint);
    currentFingerprint = nullptr;
  }
...
void MainWindowImpl::settingsTabToBack() { stopTester = true; }
...
void MainWindowImpl::scanTabToFront() {
  ...
  currentFingerprint =
      new Fingerprint(currentFinger, deviceHandler->getCurrentDevice(nullptr),
                      textLabels, iconLabels);
  disconnect(currentFingerprint->getDevice(), SIGNAL(neededStages(int)), this,
             SLOT(setScanTabNeededStages(int)));
  connect(currentFingerprint->getDevice(), SIGNAL(neededStages(int)), this,
          SLOT(setScanTabNeededStages(int)));
  disconnect(currentFingerprint, SIGNAL(neededStages(int)), this,
             SLOT(setScanTabNeededStages(int)));
  connect(currentFingerprint, SIGNAL(neededStages(int)), this,
          SLOT(setScanTabNeededStages(int)));
  disconnect(currentFingerprint->getDevice(), SIGNAL(noDeviceOpen()), this,
             SLOT(noDeviceOpen()));
  connect(currentFingerprint->getDevice(), SIGNAL(noDeviceOpen()), this,
          SLOT(noDeviceOpen()));
  disconnect(currentFingerprint->getDevice(), SIGNAL(acquireResult(int)),
             currentFingerprint, SLOT(newAcquireResult(int)));
  connect(currentFingerprint->getDevice(), SIGNAL(acquireResult(int)),
          currentFingerprint, SLOT(newAcquireResult(int)));
  disconnect(currentFingerprint->getDevice(),
             SIGNAL(verifyResult(int, struct fp_pic_data *)),
             currentFingerprint,
             SLOT(newVerifyResult(int, struct fp_pic_data *)));
  connect(currentFingerprint->getDevice(),
          SIGNAL(verifyResult(int, struct fp_pic_data *)), currentFingerprint,
          SLOT(newVerifyResult(int, struct fp_pic_data *)));
  disconnect(currentFingerprint,
             SIGNAL(acquireFinished(int, struct fp_pic_data *)), this,
             SLOT(acquireFinished(int, struct fp_pic_data *)));
  connect(currentFingerprint,
          SIGNAL(acquireFinished(int, struct fp_pic_data *)), this,
          SLOT(acquireFinished(int, struct fp_pic_data *)));
  disconnect(currentFingerprint,
             SIGNAL(verifyFinished(int, struct fp_pic_data *)), this,
             SLOT(verifyFinished(int, struct fp_pic_data *)));
  connect(currentFingerprint, SIGNAL(verifyFinished(int, struct fp_pic_data *)),
          this, SLOT(verifyFinished(int, struct fp_pic_data *)));
  if (currentFingerprint->isValid()) {
    eDialog =
        new ExistDialogImpl(tr(fingers[currentFinger]).toStdString(), this);
    eDialog->show();
    connect(eDialog, SIGNAL(choice(existDialogChoice)), this,
            SLOT(existDialogChosen(existDialogChoice)));
  } else {
    movie->setPaused(false);
    swipeFinger();
  }
...
void MainWindowImpl::settingsTabToFront() {
  ...
  if (!deviceHandler->getCurrentDevice(nullptr)->canIdentify()) {
    syslog(LOG_DEBUG,
           "The selected device doesn't support \"identification\".");
  }
}
...
void MainWindowImpl::findPamServices() {
  ...
  services[4] = (pamService){string("gnome-screensaver"), false,
                             gnomescreensaverButton, true};
